...
Form::Form()
	: _name("Default name"), _signed(false), _gradeToSign(150),
	  _gradeToExecute(150) {}
...
Form::Form(std::string name, bool sig, int gradeToSign, int gradeToExecute)
	: _name(name), _signed(sig), _gradeToSign(gradeToSign),
...
std::ostream &operator<<(std::ostream &ostm, const Form &src) {
	ostm << "Form: " << src.getName() << ": Signed: " << src.getSigned()
...
Form::GradeTooLowException::GradeTooLowException() : _msg("Grade too low") {}
